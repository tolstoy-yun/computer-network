...
int main() {
	...
	recieve_addr.sin_addr.S_un.S_addr = inet_addr("127.0.0.1");//表示IP地址的方式，这里用无符号长整型数据来表示IP地址
	...
	while (1) {
		...
		if (strcmp(recv_buf, "connect") == 0) {
			...
			cout << "发送端端口号为：" << htons(send_addr.sin_port) << endl;
			cout << "发送端IP地址为：" << inet_ntoa(send_addr.sin_addr) << endl;
			char send_ok[] = "ok";
			int sendPort_len;
		...
		*/
		if (strtol(sum_str.data(), NULL, 2) == 0) {
			...
			//如果收到的序列号大于期待的序列号，则传ACK期待的序列号-1
			else {
				cout << "收到大于期待序列号的包" << seq_us << endl;
				finalflag = 1;
				ack.push_back(finalflag + '0');
				int dseq;
				if (expectseq == 0) {
				...
				bitset<16> dseq_b = bitset<16>(dseq);//16位2进制
